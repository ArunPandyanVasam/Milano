...
const VolumePricing = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/volume-pricing", formData);
      setSuccess(true);
  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-6 rounded-xl shadow-md">
      <h2 className="text-2xl font-bold mb-4 text-indigo-600">Volume Pricing Inquiry</h2>
      {success && <p className="text-green-600 mb-4">Request sent successfully!</p>}
      {error && <p className="text-red-600 mb-4">{error}</p>}
      <form onSubmit={handleSubmit} className="space-y-4">
        {["name", "email", "phone", "company", "projectDetails"].map(field => (
          <input
            key={field}
